...
public class ParseData {
  ...
  static {
    ...
    List<MovieDetails> oldMovies =
        session.createQuery("SELECT DISTINCT movie FROM MovieDetails movie").getResultList();

    for (MovieDetails movie : oldMovies) {
      hashSet.add(movie.getFileName() + movie.getYear());
    }
  }
   ...
   * Year" of the movie and create a hash_list with it. If a entry does not exist on the hash_list
